...
if (URL[0:7] != "http://"):
  sys.exit("Only http:// is supported")
...
if ("HTTP/1.1 20" in response): #If successful connection, Find where payload starts then start receiving based on content length preferably
  ...
  if (CONTENT_LENGTH != ""):
    ...
    while (len(response) > 0):
      printable_data = response.decode()
